...
def encrypt(pub_key, encryption_key):
    z = encode(encryption_key, pub_key.bits)
    cipher_pairs = []
    for i in z:
        y = random.randint(0, pub_key.p)
        c = pow(pub_key.g, y, pub_key.p)
        d = (i * pow(pub_key.h, y, pub_key.p)) % pub_key.p
